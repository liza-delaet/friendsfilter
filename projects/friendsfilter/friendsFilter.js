...
export default class FriendsFilter {
  constructor() {
    ...
    this.bestFriendsDOMFilter = document.querySelector(
      '[data-role=filter-input][data-list=filtered]'
    );
    this.bestFriendsDOMList = document.querySelector(
      '[data-role=list-items][data-list=filtered]'
    );

    this.api = new VKAPI(51510353, 2);
  ...
  createFriendDOM(data) {
    ...
    root.innerHTML = `
    <div class="friend__avatar avatar">
      <img src="${data.photo_50}" class="avatar__img"/>
    </div>
    <div class="friend__name name">${data.first_name} ${data.last_name}</div>
  ...
  onClick(e) {
    if (e.target.dataset.role === 'list-item-swap') {
      ...
      this.move(friendId, sourceList, sourceList === 'all' ? 'filtered' : 'all');
    }
    if(e.target.dataset.role === 'app-close') {
